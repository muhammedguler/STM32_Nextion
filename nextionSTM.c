...
void sendCommand(const char* cmd)
{
   ...
   HAL_UART_Transmit (&huart1, (uint8_t*)&ENDTERMS, 3, 100);
}
...
int recvRetCommandFinished(void)
{
   ...
   int ret=0;
   uint8_t temp[4]={0};
   HAL_UART_Receive (&huart1, (uint8_t*)&temp, 4, 100);
...
int recvRetNumber(void)
{
   int number=0;
   uint8_t temp[8]={0};
   HAL_UART_Receive (&huart1, (uint8_t*)&temp, 8, 100);
...
int recvRetString(char* buffer,int len)
{
   ...
   int pointer=0, i=0;
   uint8_t c=0;
   HAL_UART_Receive (&huart1, (uint8_t*)&temp2, len+4, 100);
   for (i=0; i<len+4;  i++)
   {
      c=temp2[i];
      if (str_start_flag)
